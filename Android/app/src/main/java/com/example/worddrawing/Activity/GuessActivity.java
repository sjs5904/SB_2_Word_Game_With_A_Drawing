...
import com.example.worddrawing.Adapter.DrawingView;
...
//    String w = "ws://10.24.226.190:8080/game/"+ cid+ "/" + cname + "/" + currRoomName + "/" + currRoomID;
    ...
    private Button chatBtn, enter_btn, guess_btn;
    Dialog chatDialog;
    int mDefaultColor;
    EditText e1;
    TextView t1, server;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        guess_btn = chatDialog.findViewById(R.id.game_guess_btn);
        guess_btn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                JSONObject json = new JSONObject();
                try {
                    json.put(PROTOCOL_PREFIX, DrawingView.TYPE.CORANSWER.getVal());
                    json.put("word", e1.getText().toString());
                ...
                public void onMessage(String s) {
                    ...
                    try {
                        ...
                        if (protocol.equals(DrawingView.TYPE.DRAW_UPDATE)) {
                            ...
                            String txt = "\n" + " " + username + ": " + chat;
                            t1.append(txt);
                        } else if (protocol.equals(DrawingView.TYPE.CORANSWER)) {
                            String username = jsonString.getString("userName");
                            if (jsonString.getInt("correct") == 1) {
                                String txt = "\n" + "           " + username + " got the answer!!";
                                t1.append(txt);
                                if (jsonString.getInt("count") == 2)
                                    drawerID = jsonString.getInt("userId") + "";
                                if (jsonString.getInt("gameFinished") == 1) {
                                    txt = "\n" + "           Game Finished!";
                                    t1.append(txt);
                                    ...
                                    cc.close();
                                    Intent back = new Intent(GuessActivity.this, RoomActivity.class);
                                    startActivity(back);
                                }
                            } else if (jsonString.getInt("correct") == 0) {
                                String txt = "\n" + "           WRONG!!";
                                t1.append(txt);
                            }
                        }
